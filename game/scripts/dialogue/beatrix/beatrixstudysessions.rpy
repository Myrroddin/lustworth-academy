label beatrixbiologyessay:
    ...
    Beatrix "Oh, hey."
    __("{i}He's eyes immediately fell on the faint, reddish rash on the side of her mouth.")
    __("{i}'It looks an awful lot like cold sores.' [player_name] kept the thought to himself.{/i}")
    ...
    Jimmy __("First of all, I didn't know it had alcohol.")
    Beatrix "Well, I guess you don't know how to read, but whatever..."
    __("{i}Before [player_name] could start a round of insults to shut her down, a familiar, crisp voice cuts the air.{/i}")
    play sound "audio/sfx/stopmusiceffect.ogg"
    play music MUSIC_DAWSONS_THEME
    show missdawson neutral left with dissolve
    play sound "audio/sfx/femaleclearthroat.ogg"
    Dawson __("Beatrix! There you are. I was just looking for you.")
    Beatrix talk "Miss Dawson. Is everything alright?"
    play sound "audio/sfx/hmm01.ogg"
    Dawson __("Well, not exactly. I'm afraid your last biology test score has given your teacher, a bit of a fright.")
    Dawson __("You know we want you to succeed, especially with your aspirations.")
    Dawson __("Mr. Vivisect has agreed to let you write a detailed essay on the human reproductive system, as an opportunity to raise your grade.")
    play sound "audio/sfx/frustratedhum.ogg"
    ...
    Dawson __("That's the spirit. Now, Mr. [player_surname].")
    Dawson talk arm left __("The Headmaster is due back next week.")
    Dawson __("And he wants to have a very serious conversation with you about your... antics on Halloween night.")
    Dawson __("However, if you can help Beatrix with this essay, I will personally vouch for your good intentions when he returns.")
    Jimmy __("Seriously? Thank you. I'll do it. Absolutely.")
    play sound "audio/sfx/alright01.ogg"
    Dawson __("Alright, that's it for now.")
...
label beatrixbiologyessay02:
    ...
    __("{i}The Biology Lab had an old air conditioner that clacked once in a while but kept the room fresh.{/i}")
    play music MUSIC_BEATRIX_THEME
    ...
    Jimmy __("There is something I want to ask before we start.")
    Beatrix herpes talk "I'm listening."
    Jimmy "About that... rash thing. On your mouth."
    __("{i}Beatrix's head snaped up, her eyes narrowing. She instinctively touched the side of her mouth.{/i}")
    ...
    __("{i}Her voice got dangerously quiet, laced with an accusation that hanged heavy in the air.{/i}")
    play sound "audio/sfx/mad01.ogg"
    Beatrix mad arm "Are you seriously implying I have herpes?"
    Jimmy "Well, that looks like a cold sore, from a distance."
    __("{i}Beatrix stares at him, her anger slowly giving way to a frustrated sigh. She rubs the affected area gently.{/i}")
    play sound "audio/sfx/frustratedhum.ogg"
    ...
    __("{i}She casts her eyes down, then looks back up at him, a flicker of something softer in her gaze.{/i}")
    Beatrix "And for the record... the only person I've been THAT close to, is you."
    __("{i}A beat of surprised silence hangs between them, the fluorescent lights of the lab buzzing overhead.{/i}")
    Jimmy "I'm sorry, Beatrix."
    Beatrix "Whatever."
    __("{i}She sighs, picking up her pen and tapping it against the textbook.{/i}")
...
label beatrixbiologyessay03:
    ...
    stop music
    scene chemistrylab01 with fade
    __("{i}Beatrix was already seated near another poster of the female reproductive system propped up on the wall beside her.{/i}")
    __("{i}Her mood seems markedly better than yesterday.{/i}")
    play music MUSIC_BEATRIX_THEME
    show beatrix herpes talk with dissolve
    Jimmy "Hey!"
    play sound "audio/sfx/hey01.ogg"
    Beatrix "Oh, hey, [player_name]. You made it."
    ...
    __("{i}Beatrix lets out a small, unexpected laugh, then gives him a playful glare.{/i}")
    Beatrix "Okay, bad joke, but I'll allow it."
    Jimmy "And talking about plumbing..."
    Jimmy "Here's a tube that will help you."
    __("{i}[player_name] reached into his pocket and pulls out a small, white tube.")
    Beatrix "What's that?"
    ...
    __("{i}He gently slides the tube across the table towards her.{/i}")
    ...
    __("Beatrix picks up the tube, her fingers brushing against the label while her cheeks were blushing.")
