label missdawsonunderthedesk:
    ...
    __("{i}[player_name] immediately thought about what he saw the last time he came to the office.{/i}")
    __("{i}Maybe their 'meeting' could have something to do with the naughty business he witnessed last time?{/i}")
    __("{i}There was a way of taking advantage of the situation for [player_name].{/i}")
    __("{i}It was risky, for sure, but no risk, no fun, right? He thought.{/i}")
    ...
    Dawson "Ahh, that's good to hear. Thank you, dear. You did well."
    ...
    __("{i}[player_name] walked towards the exit making it look like he was leaving.{/i}")
    play sound "audio/sfx/highheels.ogg"
    __("{i}He listened carefully as Miss Dawson seemed to look for something in the drawers and then stood up walking to the back.{/i}")
    scene missdawsonlubeenter02 with fade
    play sound "audio/sfx/dooropen01.ogg"
    __("{i}At the exit's threshold, [player_name] heard the sound of the Headmaster's door opening.{/i}")
    play sound "audio/sfx/highheels.ogg"
    __("{i}The secretary was eagerly entering the office with some kind of bottle in her hand.{/i}")
    scene secretaryoffice01night with fade
    play sound "audio/sfx/doorclose02.ogg"
    __("{i}Once she entered, [player_name] came back in and shut the door behind him.{/i}")
    scene missdawsonhandjoblockpeek with fade
    __("{i}He then peeked through the lock and saw exactly what he expected to see.{/i}")
    __("{i}Behind the desk, Miss Dawson seemed to be trying to get under it.{/i}")
    __("{i}The plan was working so far; the only thing left to do was practice his throat clearing.{/i}")
    play sound "audio/sfx/dooropen01.ogg"
    scene headmasterstudyfallevening with fade
    play sound "audio/sfx/clearthroat01.ogg"
    __("{i}After a couple of minutes, he opened the door and cleared his throat imitating Dr. Stapleneck the best he could.{/i}")
    play sound "audio/sfx/femaleclearthroat.ogg"
    __("{i}Right away, he heard Miss Dawson also clearing her throat like a sign of approval.{/i}")
    play sound "audio/sfx/undress01.ogg"
    scene missdawsonhandjobintro01 with dissolve
    __("{i}[player_name] approached the desk and took off his sleeveless jumper, hoping that Miss Dawson wouldn't recognize his uniform from the waist down.{/i}")
    scene missdawsonhandjobintro02 with fade
    play sound "audio/sfx/slap.ogg"
    __("{i}As quickly as possible, [player_name] snuck his way to the Headmaster's chair, trying his best to hide his face.{/i}")
