...
label alicescienceprojectintro:
    ...
    __("{i}After knocking at the door, [player_name] opened and found Alice reading again in a weird pose.{/i}")
    Jimmy "You busy?"
    Alice "[player_name]! I..."
    scene alicebedroom with fade
    show alice casual blush left with dissolve
    play sound "audio/sfx/hey02.ogg"
    __("{i}She jumped on the bed, untangling her legs from a position that looked like a human pretzel.{/i}")
    ...
    Alice "Wait here..."
    __("{i}Alice went to the back of the tree and retrieved a strange artifact. As she got closer, [player_name] realized what it was.{/i}")
...
label alicescienceprojectoutro:
    ...
    play music MUSIC_ALICES_THEME
    scene alicereadingfront with fade
    __("{i}Alice was back in that same position, head hanging toward the floor, reading a book about propulsion.{/i}")
    scene alicereadingback with fade
    __("{i}[player_name] tried to focus, but the image of Sally emerging from the lake was burned into his retinas.{/i}")
    __("{i}His brain was doing laps around her curves, flashing images of Sally while Alice moved her spreaded legs on the bed.{/i}")
    play sound "audio/sfx/oh1.ogg"
    scene aliceintrusivethought01 with vpunch
    __("{i}The intrusive thought of getting his cock out and fucking Alice right there on top of the bed in that same position, took over everything else.{/i}")
    ...
    show alice casual blush left with dissolve
    play sound "audio/sfx/hey02.ogg"
    __("{i}Alice flips upright, her eyes wide with excitement.{/i}")
    __("{i}She doesn't even notice [player_name] was standing there in a strange pose, trying to hide a certain 'situation.'{/i}")
    ...
    __("{i}The two space engineers spent the next hour mixing the 'biofuel.'{/i}")
    __("{i}It was a disgusting slurry of brown and grey, but Alice handled it like she was working with liquid gold.{/i}")
    scene alicescienceprojectintro with fade
    ...
    __("{i}For three seconds, nothing happened.{/i}")
    scene rocketlaunch01 with vpunch
    play sound "audio/sfx/rocketboost.ogg"
    __("{i}Then, the rocket started vibrating like an angry hornet.{/i}")
    __("{i}A cloud of thick, brown smoke—smelling really bad engulfed the yard.{/i}")
    scene rocketlaunch02 with vpunch
    __("{i}With a sound like a wet firecracker, the 'Sterculius-1' screeched into the sky.{/i}")
    __("{i}It was a chaotic, spiraling mess of smoke and shit-fumes.{/i}")
    Alice "It's doing it! It's!"
    play sound "audio/sfx/rocketfall01.ogg"
    scene rocketlaunch03 with vpunch
    __("{i}The rocket reached about fifty feet, sputtered a jet of brown flame, and then did a sharp, 180-degree turn.{/i}")
    __("{i}It wasn't going to the stars; it was coming for Alice.{/i}")
    Jimmy "Look out!"
    play sound "audio/sfx/big_punch.ogg"
    __("{i}[player_name] tackled her into the grass just as the rocket hissed over their heads, embedding itself three inches deep into the wooden porch.{/i}")
    scene alicerocketlaunchending with vpunch
    play sound "audio/sfx/slap.ogg"
    __("{i}Alice was pinned under [player_name], in a very uncomfortable position.{/i}")
    __("{i}He could smell her hair and it smelled good, and she could definitely smell the... well, everything he smelled like at that point.{/i}")
    Alice "..."
    Jimmy "Houston, we have a problem?"
    __("{i}Her eyes were wide, blinking fast, and her face was turning a shade of red that rivaled the not-rotten apples.{/i}")
    ...
    __("{i}[player_name] scrambled up, dusting off his knees and tried to regain some of his 'tough-guy' composure.{/i}")
    ...
    if quests.antonellaBeachFun == COMPLETE and quests.blairDrunk == COMPLETE and quests.kassandraBathtubClimax == COMPLETE:
        ...
        $ renpy.pause()
        __("{i}You have reached the end of v0.5.5 Extended Edition. Beyond this point, the game hasn't been updated.{/i}")
        __("{i}Work on v0.5.6 is currently ongoing with new content for the Ranch and more content for the weekends.{/i}")
