label beatrix_beatrixboobjob_scene:
    play music MUSIC_SEXY_THEME
    scene beatrixvaginalesson with fade
    Beatrix "Here's my specimen."
    Jimmy "Uh, looks tasty."
    play sound "audio/sfx/frustratedhum.ogg"
    Beatrix "UGHHH GROSS!"
    Beatrix "Just concentrate, alright?"
    Beatrix "The little dot we have up here, it's called the clitoris. It's basically a mini penis, that if it gets rubbed, we girls get aroused."
    Jimmy "Mine is bigger, HAHAHAHAHAHA!" with vpunch
    play sound "audio/sfx/laugh03.ogg"
    Beatrix "Ha, ha, ha, I can't believe I feel for that."
    Beatrix "So, going down from the clit, there is a small hole called urethra, that's where the pee comes out."
    Jimmy "Ugh, gross."
    Beatrix "And down there, the bigger hole is the actual vaginal opening."
    Jimmy "Oh, yeah, that's were I come in."
    Beatrix "Yeah, in your dreams..."
    play sound "audio/sfx/gasp02.ogg"
    Beatrix "ARE YOU SERIOUS?" with vpunch
    Beatrix "What is that bulge in your pants?"
    Jimmy "Come on, I'm human. I'm looking at your pussy and that makes me hard."
    Beatrix "Of course, the male primal instincts."
    Beatrix "You know what?"
    Beatrix "You are lucky I need more of you cum, to run some tests."
    scene beatrixboobjobintro with fade
    play sound "audio/sfx/undress01.ogg"
    "{i}Beatrix got down the table and started undressing her chest.{/i}"
    "[player_name]'s manhood was already hard for the occasion and Beatrix seemed impressed with it."
    play sound "audio/sfx/mh1.ogg"
    Beatrix "Are these things supposed to be this big?"
    Jimmy "Oh, yeah."
    Beatrix "Good thing, my boobs have a good size."
    play sound "audio/sfx/handjob01.ogg"
    scene beatrixboobjobanim with fade
    "[player_name] could feel her warm skin around his cock as she started to work it up and down like a natural."
    "He could feel her boobs bouncing on his hip as she clenched his cock more and more each time."
    Jimmy "Oh, fuck. I never thought you would be so good at this!"
    play sound "audio/sfx/ah1.ogg"
    Beatrix "I've seen stuff on the internet."
    "{i}The sensation of her tits being fucked by his cock was unlike anything she had ever experienced before.{/i}"
    "{i}He grabbed the edge of the desk, feeling that he was getting closer and closer to the climax.{/i}"
    Jimmy "Oh fuck, here it comes!"
    scene beatrixboobjobcum with vpunch
    play sound "audio/sfx/cumshotone.ogg"
    "{i}He grunted as he felt his cum shoot in all directions.{/i}"
    Jimmy "Holy shit, that felt so good."
    Jimmy "I love those tits."
    play sound "audio/sfx/giggle01.ogg"
    Beatrix "I like your cock too, he, he."
    return

image beatrixboobjobanim:
    "beatrixboobjobanim01"
    pause 0.1
    "beatrixboobjobanim02"
    pause 0.1
    "beatrixboobjobanim03"
    pause 0.1
    "beatrixboobjobanim04"
    pause 0.1
    "beatrixboobjobanim05"
    pause 0.1
    "beatrixboobjobanim06"
    pause 0.1
    "beatrixboobjobanim07"
    pause 0.1
    "beatrixboobjobanim08"
    pause 0.1
    "beatrixboobjobanim09"
    pause 0.1
    "beatrixboobjobanim10"
    pause 0.1
    "beatrixboobjobanim11"
    pause 0.1
    "beatrixboobjobanim12"
    pause 0.1
    "beatrixboobjobanim13"
    pause 0.1
    "beatrixboobjobanim14"
    pause 0.1
    "beatrixboobjobanim13"
    pause 0.1
    "beatrixboobjobanim12"
    pause 0.1
    "beatrixboobjobanim11"
    pause 0.1
    "beatrixboobjobanim10"
    pause 0.1
    "beatrixboobjobanim09"
    pause 0.1
    "beatrixboobjobanim08"
    pause 0.1
    "beatrixboobjobanim07"
    pause 0.1
    "beatrixboobjobanim06"
    pause 0.1
    "beatrixboobjobanim05"
    pause 0.1
    "beatrixboobjobanim04"
    pause 0.1
    "beatrixboobjobanim03"
    pause 0.1
    "beatrixboobjobanim02"
    pause 0.1
    repeat

