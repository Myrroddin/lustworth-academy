label eunice_fuckvision_scene:
    play music MUSIC_SEXY_THEME
    scene eunicefuckvisionanim with fade
    play sound "audio/sfx/cowgirl01.ogg"
    __("{i}Suddenly, his mind is transported to a different place.{/i}")
    __("{i}He feels pleasure as he sees himself having sex with a woman.{/i}")
    __("{i}The woman was big sized. Her whole body shaked wildly with every thrust of his cock.{/i}")
    __("{i}After a couple of seconds of pulling and retreating, he realizes who the woman was...{/i}")
    Eunice "My god! [player_name]! This is the best feeling I've ever had in my life!"
    Eunice "I thought eating a triple smash burger was my happy place, but..."
    Eunice "This is so much better!"
    Eunice "It feels like a flaming stick inside my fat pussy!"
    play sound "audio/sfx/cowgirl02.ogg"
    scene eunicefuckvisionanimfast with vpunch
    Eunice "Awww, YES! Faster, faster, faster!"
    Eunice "Uff, I feel like I'm being impaled..."
    Eunice "Come on! Get that cum inside me!"
    Eunice "I want to have ten kids with you!"
    Jimmy "What!?" with vpunch
    play sound "audio/sfx/cowgirlcum.ogg"
    Eunice "And we can adopt ten more from all over the world!"
    Eunice "Get that cum real deep inside!"
    Jimmy "Wait, no. I don't wanna..."
    Eunice "DOOOO IT!" with vpunch
    __("{i}[player_name] couldn't resist the heat of the moment.{/i}")
    scene eunicefuckvisioncum with vpunch
    play sound "audio/sfx/cumshotone.ogg"
    Eunice "OH MY GOD!"
    Eunice "That feels... so good."
    Eunice "Your cum is so warm..."
    stop music
    play sound "audio/sfx/stopmusiceffect.ogg"
    play music "audio/music/funnymoment.ogg"
    Eunice "We are going to have so many children!"
    Jimmy "Oh, no, please, no..."
    play sound "audio/sfx/babynightmare.ogg"
    show babynightmare with dissolve
    Eunice "Do you hear that? Those are our kids!"
    Jimmy "What the fuck? NO!" with vpunch
    Eunice "HAHAHAHAHAHAHAHAHA!" with vpunch
    Jimmy "No, no, no..."
    Eunice "We are having twins, [player_name]!"
    Jimmy "NOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO!" with vpunch
    if eunicegallery == True:
        call screen eunicegallery
    return

image eunicefuckvisionanim:
    "eunicefuckvision01"
    pause 0.1
    "eunicefuckvision02"
    pause 0.1
    "eunicefuckvision03"
    pause 0.1
    "eunicefuckvision04"
    pause 0.1
    "eunicefuckvision05"
    pause 0.1
    "eunicefuckvision06"
    pause 0.1
    "eunicefuckvision07"
    pause 0.1
    "eunicefuckvision08"
    pause 0.1
    "eunicefuckvision09"
    pause 0.1
    repeat

image eunicefuckvisionanimfast:
    "eunicefuckvision01"
    pause 0.08
    "eunicefuckvision02"
    pause 0.08
    "eunicefuckvision03"
    pause 0.08
    "eunicefuckvision04"
    pause 0.08
    "eunicefuckvision05"
    pause 0.08
    "eunicefuckvision06"
    pause 0.08
    "eunicefuckvision07"
    pause 0.08
    "eunicefuckvision08"
    pause 0.08
    "eunicefuckvision09"
    pause 0.08
    repeat
