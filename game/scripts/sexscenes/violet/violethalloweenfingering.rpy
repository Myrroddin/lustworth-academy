label violet_halloweenfingering_scene:
    play music MUSIC_SEXY_THEME
    scene violetcaught02 with dissolve
    Violet "Oh, Derby. You returned quickly."
    Jimmy "..."
    Violet "The room is a little warmer than usual..."
    Violet "There has been a while since we do dirty stuff."
    Violet "That's why I wanted to sleep with you tonight."
    play sound "audio/sfx/giggle01.ogg"
    Violet "I wanted you to see if we could maybe touch each other..."
    Violet "Derby, can you touch my vagina?"
    "*Oh, fuck. How do I get out of this?*"
    Violet "Derby, don't you want me anymore?"
    Violet "I'm getting wet down there, please. Give me your hand."
    scene violetfingeringanim with dissolve
    play sound "audio/sfx/oh1.ogg"
    Violet "See? My pussy is so warm."
    Jimmy "Uhmm."
    play sound "audio/sfx/handjob01.ogg"
    Violet "Uff, you like it, don't you? I can tell."
    Violet "I know you like me playing like this with your butt, but today you gotta please your princess."
    "*Da fuq?*"
    scene violetfingeringanimfast with vpunch
    Violet "Oh, yes! Fuck, that feels so good."
    Violet "Derby, your fingers reach further inside me than your penis."
    Violet "Oh! Where did you learn to move your fingers like that!"
    Violet "I think I'm gonna cum!"
    play sound "audio/sfx/ah2.ogg"
    scene violetfingeringcum with vpunch
    Violet "Oh... my... god..."
    Violet "Was that an orgasm?"
    Violet "That's the first time you've made me cum, Derby!"
    play sound "audio/sfx/mh1.ogg"
    Violet "Get your cock out, now! I want to have sex!"
    Jimmy "Umm..."
    Violet "Do it, now! Your princess demands it!"
    scene violetcaught06 with fade
    play sound "audio/sfx/undress01.ogg"
    "{i}Being so horny. [player_name] thought, \"fuck it!\" and pulled out his cock forgetting about his mission.{/i}"
    Violet "Wow! Derby? Did you inyect botox in your penis?"
    play sound "audio/sfx/big_punch.ogg"
    Violet "I don't remember it being so big... "
    scene violetcaught07 with vpunch
    play sound "audio/sfx/stopmusiceffect.ogg"
    play music "audio/music/crazymoment01.ogg"
    "{i}Indeed. The real Derby opened the door as Violet was admiring [player_name]'s manhood with her mouth wide open.{/i}"
    Derby "Violet?"
    Derby "WHAT THE FUCK IS THIS?!"
    $ derbylaptopfound = True
    if violetgallery == True:
        call screen violetgallery
    return

image violetfingeringanim:
    "violetfingeringanim01"
    pause 0.1
    "violetfingeringanim02"
    pause 0.1
    "violetfingeringanim03"
    pause 0.1
    "violetfingeringanim04"
    pause 0.1
    "violetfingeringanim05"
    pause 0.1
    "violetfingeringanim06"
    pause 0.1
    "violetfingeringanim07"
    pause 0.1
    "violetfingeringanim08"
    pause 0.1
    "violetfingeringanim09"
    pause 0.1
    "violetfingeringanim08"
    pause 0.1
    "violetfingeringanim07"
    pause 0.1
    "violetfingeringanim06"
    pause 0.1
    "violetfingeringanim05"
    pause 0.1
    "violetfingeringanim04"
    pause 0.1
    "violetfingeringanim03"
    pause 0.1
    "violetfingeringanim02"
    pause 0.1
    repeat

image violetfingeringanimfast:
    "violetfingeringanim01"
    pause 0.06
    "violetfingeringanim02"
    pause 0.06
    "violetfingeringanim03"
    pause 0.06
    "violetfingeringanim04"
    pause 0.06
    "violetfingeringanim05"
    pause 0.06
    "violetfingeringanim06"
    pause 0.06
    "violetfingeringanim07"
    pause 0.06
    "violetfingeringanim08"
    pause 0.06
    "violetfingeringanim09"
    pause 0.06
    "violetfingeringanim08"
    pause 0.06
    "violetfingeringanim07"
    pause 0.06
    "violetfingeringanim06"
    pause 0.06
    "violetfingeringanim05"
    pause 0.06
    "violetfingeringanim04"
    pause 0.06
    "violetfingeringanim03"
    pause 0.06
    "violetfingeringanim02"
    pause 0.06
    repeat