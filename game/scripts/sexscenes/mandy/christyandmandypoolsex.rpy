label christy_halloweenpoolsex_scene:
label mandy_halloweenpoolsex_scene:
    play music MUSIC_SEXY_THEME
    scene christymandypool01 with fade
    Christy "Fuck, I'm sweating already!"
    Christy "This thing is so good!"
    Christy "Come on, Mandy! What are you waiting for?"
    __("{i}Hesitating, Mandy took her skirt off and started to untie her bikini while looking at [player_name], kind of ashamed.{/i}")
    __("{i}But, she didn't stop.{/i}")
    scene christymandypool02 with fade
    Mandy "You better not say a word about this to my boyfriend, Christy."
    Christy "That half-brain quarterback? He wouldn't understand a word."
    __("{i}Mandy looked at [player_name], waiting for a reply.{/i}")
    Jimmy "I was never here."
    Mandy "Alright..."
    scene christymandypool03 with fade
    Christy "I told you, Mandy. You're already so wet down there."
    Christy "I bet you're enjoying the view, [player_name]."
    Jimmy "..."
    Christy "Good boy. Don't worry, Mandy, he won't say a thing."
    __("{i}It was the truth. [player_name] felt so aroused by looking a two of the hottest girls in school naked for him.{/i}")
    __("{i}But he was a man of his word. However, being in the pool wasn't exactly a hidden spot, so he was afraid of being seen already.{/i}")
    scene christymandypool04 with fade
    Christy "Mandy, you truly have the nicest ass I've ever seen."
    Mandy "You have said that like a thousand times."
    Christy "Because it's true."
    Christy "And look at that cute pinky pussy..."
    Christy "Fuck, I want to eat it all!"
    Mandy "AHHH, Oh my god, Christy!"
    Christy "I'm always surprised of how tight you are."
    Christy "Teddy's dick must be a joke."
    Mandy "{i}*Giggles*{/i}"
    Mandy "FUUUUCK! Just like that!"
    Mandy "I'm gonna CUM!"
    scene christymandypool04 with vpunch
    pause 0.8
    scene christymandypool05 with fade
    Christy "That's right, girl. Now it's your turn to make me cum!"
    Christy "OHH SHIT!!"
    Christy "You're such an expert with that tongue!"
    Christy "Who needs a dick, when I have you, Mandy."
    Mandy "Ghusgassd..."
    Christy "That's right... Give it to me! I'm cumming!"
    scene christymandypool05 with vpunch
    Christy "OOOOOOOHHHH FUUUCK!"
    scene christymandypoolcum with fade
    Christy "That was amazing."
    Mandy "Yeah, it was good."
    if christygallery == True:
        call screen christygallery
    elif mandygallery == True:
        call screen mandygallery
    return
