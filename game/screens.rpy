...
screen say(who, what):
    ...
    ## If there's a side image, display it. Do not display on the
...
default persistent.quick_menu_opacity = 50
define quick_menu_fade_time = 0.25

transform quick_menu_fade_in:
    alpha (persistent.quick_menu_opacity / 100)
    easein quick_menu_fade_time alpha 1.0

transform quick_menu_fade_out:
    alpha 1.0
    easein quick_menu_fade_time alpha (persistent.quick_menu_opacity / 100)
...
screen quick_menu():
    ...
    if quick_menu and persistent.quick_menu:
        fixed:
            ...
            if quick_menu_hovered:
                at quick_menu_fade_in
            else:
                at quick_menu_fade_out
            
            mousearea:
                if not quick_menu_expanded:
                    focus_mask 'quick_menu_draft_closed_ground'
                hovered SetScreenVariable('quick_menu_hovered', True)
                unhovered SetScreenVariable('quick_menu_hovered', False)

            if quick_menu_expanded:
                imagemap:
                    ground 'quick_menu_draft_open_ground'
                    insensitive Fixed()
                    idle Fixed()
                    hover 'quick_menu_draft_open_hover'
                    alpha False

                    hotspot (24, 28, 50, 50) action Skip() alternate Skip(fast=True, confirm=True)
                    hotspot (24, 83, 50, 50) action QuickSave()
                    hotspot (24, 138, 50, 50) action QuickLoad()
                    hotspot (24, 192, 50, 50) action ShowMenu('preferences')
                    hotspot (105, 68, 62, 44) action Rollback()
                    hotspot (190, 107, 40, 58) action Preference('auto-forward', 'toggle')
                    hotspot (224, 178, 50, 50) action ShowMenu('history')
                    hotspot (84, 182, 20, 52) action ToggleScreenVariable('quick_menu_expanded')
            else:
                imagemap:
                    ground 'quick_menu_draft_closed_ground'
                    insensitive Fixed()
                    idle Fixed()
                    hover 'quick_menu_draft_closed_hover'
                    alpha False

                    hotspot (10, 186, 50, 50) action ShowMenu('preferences')
                    hotspot (73, 186, 50, 50) action QuickLoad()
                    hotspot (137, 186, 50, 50) action QuickSave()
                    hotspot (208, 186, 50, 50) action Skip() alternate Skip(fast=True, confirm=True)
                    hotspot (220, 142, 52, 20) action ToggleScreenVariable('quick_menu_expanded')

# screen quick_menu():
#     ## Ensure this appears on top of other screens.
#     zorder 100

#     $ path = 'gui/web/quick_menu/' if renpy.variant('web') else 'gui/quick_menu/'
#     if quick_menu and persistent.quick_menu:
#         frame:
#             background Frame(path + 'lowbuttonsbox.webp')
#             padding gui.quick_menu_padding
#             align 0.5, 1.0

#             hbox:
#                 spacing gui.quick_menu_spacing

#                 imagebutton:
#                     idle (path + 'saveminibuttonidle.webp')
#                     hover (path + 'saveminibuttonhover.webp')
#                     action ShowMenu('save')
#                     #tooltip "Save"

#                 imagebutton:
#                     idle (path + 'backminibuttonidle.webp')
#                     hover (path + 'backminibuttonhover.webp')
#                     action Rollback()
#                     #tooltip "Back"

#                 imagebutton:
#                     idle (path + 'historyminibuttonidle.webp')
#                     hover (path + 'historyminibuttonhover.webp')
#                     action ShowMenu('history')
#                     #tooltip "History"

#                 imagebutton:
#                     idle (path + 'skipminibuttonidle.webp')
#                     hover (path + 'skipminibuttonhover.webp')
#                     action Skip() alternate Skip(fast=True, confirm=True)
#                     #tooltip "Skip"

#                 imagebutton:
#                     idle (path + 'quicksaveminibuttonidle.webp')
#                     hover (path + 'quicksaveminibuttonhover.webp')
#                     action QuickSave()
#                     #tooltip "Quick Save"

#                 imagebutton:
#                     idle (path + 'quickloadminibuttonidle.webp')
#                     hover (path + 'quickloadminibuttonhover.webp')
#                     action QuickLoad()
#                     #tooltip "Quick Load"

#                 imagebutton:
#                     idle (path + 'settingsminibuttonidle.webp')
#                     hover (path + 'settingsminibuttonhover.webp')
#                     action ShowMenu('preferences')
#                     #tooltip "Options"

#                 # imagebutton:
#                 #     idle (path + 'patreonminibuttonidle.webp')
#                 #     hover (path + 'patreonminibuttonhover.webp')
#                 #     action OpenURL("https://www.patreon.com/impactxplay")

#         # TODO: Revisit; maybe add a timer or make toggleable
#         # $ tooltip = GetTooltip()
#         # if tooltip:
#         #     nearrect:
#         #         focus 'tooltip'
...
screen main_menu():
    ...
    imagemap:
        ...
        $ f = lambda *x: _scale(*x)
        hotspot f(133, 335, 610, 116)  action Start()
        hotspot f(242, 514, 608, 116)  action ShowMenu("load")
        hotspot f(355, 697, 611, 116)  action ShowMenu("preferences")
        hotspot f(1735, 943, 155, 117) action ShowMenu("about")
        hotspot f(1190, 885, 489, 128) action SetVariable('galleryMode', True), Start("jimmyspc")
        hotspot f(42, 912, 311, 132)   action OpenURL("https://www.patreon.com/impactxplay")
        hotspot f(480, 884, 610, 116)  action Quit(confirm=False)
...
style return_button:
    xpos gui.navigation_xpos
    yalign 1.0
    yoffset gui.return_button_ypos
            ...
            grid gui.file_slot_cols gui.file_slot_rows:
                ...
                for i in range(gui.file_slot_cols * gui.file_slot_rows):
                    ...
                    button:
                        ...
                        null height 6
                        add Transform(FileScreenshot(slot), xysize=(config.thumbnail_width, config.thumbnail_height)):
                            xpos gui.thumbnail_xpos
                            ypos gui.thumbnail_ypos
                        null height 20
    ...
    use game_menu(_("Preferences"), scroll="viewport"):
        ...
        vbox:
            ...
            hbox:
                style_prefix "slider"
                box_wrap True
                box_wrap_spacing (4 * gui.pref_spacing)
...
screen chapterchoiceskip():
    imagemap:
        ground "chapterchoice01"
        hover "chapterchoice02"

        hotspot (310, 202, 1265, 330) clicked Return("prologue")
        hotspot (310, 606, 1271, 292) clicked Return("chapterone")
        hotspot (1626, 813, 271, 267) clicked Return("halloween")
